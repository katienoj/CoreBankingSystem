...
<c:choose>
	<c:when test="${data.account_id > 0 }">
		<h3>
			...
			<a
				href="${page.url_host}${page.url_apppath}admin/loan/details/${data.account_id}"
				class="btn btn-small btn-info pull-right" style="margin-top: 15px;">
				View Details of Loan Account</a>
		</h3>
	</c:when>
	<c:otherwise>
		<h3>Add New Loan</h3>
		...
		<tr>
			...
			<td><select name="account_type" placeholder="Account Type">
					...
					<option
						...
						value="1">Normal Loan</option>
					<option
						<c:if test="${data.account_type == 2}">selected="selected"</c:if>
						value="2">Development Loan</option>
                                        <option
						<c:if test="${data.account_type == 3}">selected="selected"</c:if>
						value="3">Emergency Loan</option>
					<option
						<c:if test="${data.account_type == 4}">selected="selected"</c:if>
						value="4">Education Loan</option>
			</select></td>
		</tr>
		<tr>
			<td>Bank Branch</td>
