...
import business_controllers.LoanController;
	...
	public String dashboard(HttpServletRequest request, HttpServletResponse response,
		...
		if(uris.get("uri_2").equals("account")){
			...
			if(uris.get("uri_3").equals("new")){
				...
				page.setPage_title("New Loan");
				page.setViewFile("admin/loan_edit.jsp");
			}
			if(uris.get("uri_3").equals("edit")){
				...
				try{
					...
					page.setPage_title("Edit Loan");
					page.setViewFile("admin/loan_edit.jsp");
				} catch (NumberFormatException e){
					e.printStackTrace();
					message_queue.add(new Message("Loan ID is not valid.", "error"));
					redirectAttributes.addFlashAttribute("message_queue", message_queue);
					return "redirect:/admin/loan/search";
					
				} catch (NotFoundException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					message_queue.add(new Message("Loan Not Found", "error"));
					redirectAttributes.addFlashAttribute("message_queue", message_queue);
					return "redirect:/admin/loan/search";
				}
			...
			//Loan search page
			if(uris.get("uri_3").equals("search")){
				page.setPage_title("Search Loan");
				page.setViewFile("admin/loan_search.jsp");
			}
			//Loan search page
			if(uris.get("uri_3").equals("search_loan")){
				int account_id = Integer.parseInt(uris.get("uri_4"));
				Account data;
				try {
					data = accountController.getAccountDetails(account_id);
					model.addAttribute("data", data);
					page.setPage_title("Loan Details");
					page.setViewFile("admin/loan_details.jsp");
				} catch (NotFoundException e) {
					e.printStackTrace();
					message_queue.add(new Message("Loan Account Does Not Exist", "error"));
					redirectAttributes.addFlashAttribute("message_queue", message_queue);
					return "redirect:/admin/dashboard";
				}
