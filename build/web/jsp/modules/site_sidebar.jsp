				...
				<li><a
					href="${page.url_host}${page.url_apppath}session/account/${_account.account_id}">
						ID : ${_account.account_id} <c:choose>
							<c:when test="${_account.account_type == 1 }"> - Transactional Account </c:when>
							<c:when test="${_account.account_type == 2 }"> - Savings Account </c:when>
						        <c:when test="${_account.account_type == 3 }"> - Joint Account </c:when>
						        <c:when test="${_account.account_type == 4 }"> - Corporate Account </c:when>
			...
			<li class='last'><a
					...
					<table>
						<tr>
							<td><object width="25px" height="25px"
									...
									type="image/svg+xml"></object></td>
							<td><span class="menu_link">Account Statement</span></td>
						</tr>
					</table>
			</a></li>
				...
				href='${page.url_host}${page.url_apppath}bank_account/transfer_history'>
					<table>
						<tr>
							<td><object width="25px" height="25px"
									data="${page.url_assets}images/bank_atm.svg"
									type="image/svg+xml"></object></td>
							<td><span class="menu_link">Loans</span></td>
						</tr>
					</table>
			</a></li>
